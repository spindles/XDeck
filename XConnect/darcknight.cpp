...
#include "ui_darcknight.h"
#include <stdio.h>
#include <iostream>
#include <sstream>

DarcKnight::DarcKnight(CharacterSheet& cs, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::DarcKnight)
{
    ui->setupUi(this);
    running = false;
    csLocal = &cs;
...
DarcKnight::~DarcKnight()
{
    delete ui;
}

void DarcKnight::changeEvent(QEvent *e)
{
...
void DarcKnight::on_pushButtonLaunch_released()
{
    if (running==false)
    {
        running = true;
...
void DarcKnight::setupProgramOutput()
{
    int ip1, ip2, ip3, ip4;
    ...
    textToPrint = "Launching D'Arc Knight at ip: ";
...
void DarcKnight::updateText()
{
    if(counter==textToPrint.length())
    {
        ui->plainTextEditOutput->textCursor().insertText("Done!");
